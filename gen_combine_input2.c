...
float para_val_ht       [10] ;
float para_err_ht_fit   [10] ;    
float para_err_ht_syst  [10] ;

float para_val_njet     [10] ;    
float para_err_njet_fit [10] ;
float para_err_njet_syst[10] ;
 
float para_val_ht_mht[10][10] ;
float para_err_ht_mht[10][10] ;

float para_val_nb       [10] ;
float para_err_nb       [10] ;
    
float para_rel_err_ht   [10] ;
float para_rel_err_njet [10] ;
float para_rel_err_ht_mht[10][10] ;       
float para_rel_err_nb   [10] ;    
...
int find_new_region_bi_no ( int bi_nj, int bi_nb, int bi_mht, int bi_ht );
                       ...
                       ) 
{
   ...
   for ( int bg = 0; bg < 4; bg++)
   {
	 ...
	 TString ht_str;
      ...
      for ( int bi_ht = 1; bi_ht<=nBinsHT; bi_ht++ )
         fprintf( ofp_combine, "Kht%d   ", bi_ht);

      fprintf( ofp_combine, " ");
      for ( int bi_nj = 1; bi_nj<=nb_nj; bi_nj++ )
	 if ( bi_nj != njet_bin_to_fix_in_qcd_model_fit+1 )
            fprintf( ofp_combine, "Snj%d   ", bi_nj);
      ...
      for ( int bi_nj=1; bi_nj<=nb_nj; bi_nj++ ) {
         for ( int bi_nb=1; bi_nb<=nb_nb; bi_nb++ ) {
            for ( int bi_htmht=1; bi_htmht<=nb_htmht; bi_htmht++ ) {
               ...
               htmht_bin_to_ht_mht_bins ( bi_htmht, bi_ht, bi_mht );
              ...
              //-------
               ...
               region_bi = find_new_region_bi_no(bi_nj, bi_nb, bi_mht, bi_ht); /// Because new binning is not compatible with text files we have right now.
               ...
               for ( int bg = 0; bg < 3; bg++)
	       {
               ...
               printf( " %s : %3d %s %3d  %s  Nldp = %7.1f +/- %5.1f +/- %5.1f ,  Nhdp = %7.1f +/- %5.1f +/- %5.1f\n",
		  ...
		  Rqcd_val = para_val_ht[bi_ht] * para_val_njet[bi_nj] * para_val_ht_mht[bi_ht][bi_mht-1] ;
                  ...
                  rel_err_ht[bi_ht] += para_rel_err_ht[bi_ht] ;

		  std::vector <float> rel_err_nj(10,1.);// creating a [10] vector with inital value 1.
                  if ( bi_nj != njet_bin_to_fix_in_qcd_model_fit+1 ) rel_err_nj[bi_nj] += para_rel_err_njet[bi_nj] ;

		  std::vector<std::vector<float>> rel_err_ht_mht(10,std::vector<float>(10,1.)) ; //creating a [10][10] vector with inital value 1.
                  rel_err_ht_mht[bi_ht][bi_mht-1] += para_rel_err_ht_mht[bi_ht][bi_mht] ;

		  std::vector <float> rel_err_nb(10,1.);
                  rel_err_nb[bi_nb-1] += para_rel_err_nb[bi_nb] ;
                  ...
                  for ( int bi_ht = 1; bi_ht<=nBinsHT; bi_ht++ )
                     Rqcd_rel_err2 += pow( rel_err_ht[bi_ht]-1, 2. ) ;
		  ...
		  for ( int bi_nb =1; bi_nb<=nb_nj; bi_nb++ )
                  ...
                  fprintf( ofp_qcdratio, " %3d %28s  Kht%d * Snj%d * Smht%d = (%6.4f +/- %6.4f) * (%6.4f +/- %6.4f) * (%6.4f +/- %6.4f) = %7.4f +/- %7.4f\n",
                      ...
                      para_val_ht[bi_ht], sqrt( pow( para_err_ht_fit[bi_ht], 2.) + pow( para_val_ht[bi_ht]*para_err_ht_syst[bi_ht], 2.) ),
                      para_val_njet[bi_nj], sqrt( pow( para_err_njet_fit[bi_nj], 2.) + pow( para_val_njet[bi_nj]*para_err_njet_syst[bi_nj], 2.) ),
                      para_val_ht_mht[bi_ht][bi_mht-1], para_err_ht_mht[bi_ht][bi_mht-1],
                      Rqcd_val, Rqcd_err ) ;
                  ...
                  for ( int bi_ht =1; bi_ht<=nBinsHT; bi_ht++ )
		  {
                     ...
                     {
                        if ( bi_ht == 1 && bi_mht > 2 ) continue;
                        if ( rel_err_ht_mht[bi_ht][bi_mht] == 1. ) { fprintf( ofp_combine, "   -   " ) ; } 
			        else { fprintf( ofp_combine, " %6.3f", rel_err_ht_mht[bi_ht][bi_mht] ) ; }
		     }
                     if ( bi_ht != nBinsHT ) fprintf( ofp_combine, " " ) ;		     
		  }
                     fprintf( ofp_combine, " " ) ;
                     ...
                     for ( int bi_ht =1; bi_ht<=nBinsHT; bi_ht++ )   
                        fprintf( ofp_combine, " %6.3f", rel_err_ht[bi_ht] ) ;
...
void read_pars( const char* model_pars_file ) {
   ...
   for ( int bi_ht = 1; bi_ht<=nBinsHT; bi_ht++ )
   {
      ...
      para_val_ht     [bi_ht] = val ;
      para_err_ht_fit [bi_ht] = err1 ;
      para_err_ht_syst[bi_ht] = val*err2 ;
      para_rel_err_ht [bi_ht] = rel_err ;
   ...
   for ( int bi_njet=1; bi_njet<=nb_nj; bi_njet++ )
   {
      ...
      get_par( ifs_model_pars, pname, val, err1, err2, rel_err ) ;
      para_val_njet     [bi_njet] = val ;
      para_err_njet_fit [bi_njet] = err1 ;
      para_err_njet_syst[bi_njet] = val*err2 ;
      para_rel_err_njet [bi_njet] = rel_err ;
      printf("   Read %s : %.4f %.4f %.4f\n", pname, val, err1, err2 ) ;
   }//bi_njet
       //---
   ...
   for ( int bi_ht = nBinsHT; bi_ht>=1; bi_ht-- )
      for ( int bi_mht=0; bi_mht<nb_mht; bi_mht++ )
      {
         ...
         get_par( ifs_model_pars, pname, val, err1, err2, rel_err ) ;
         para_val_ht_mht    [bi_ht][bi_mht+1] = val ;
         para_err_ht_mht    [bi_ht][bi_mht+1] = sqrt( err1*err1 + val*val*err2*err2 ) ;
         para_rel_err_ht_mht[bi_ht][bi_mht+1] = rel_err ;
         printf("   Read %s : %.4f %.4f %.4f\n", pname, val, err1, err2 ) ;
      }//bi_mht and bi_ht
   ...
   for ( int bi_nb=1; bi_nb<=nb_nb; bi_nb++ )
   {
      ...
      get_par( ifs_model_pars, pname, val, err1, err2, rel_err ) ;
      para_val_nb    [bi_nb] = val ;
      para_err_nb    [bi_nb] = sqrt ( err1*err1 + val*val*err2*err2 ) ;
      para_rel_err_nb[bi_nb] = rel_err ;
      printf("   Read %s : %.4f %.4f %.4f\n", pname, val, err1, err2 ) ;
   }//bi_nb
  ...
  //=======================================================================================
   ...
   int find_new_region_bi_no ( int bi_nj, int bi_nb, int bi_mht, int bi_ht ) {
      ...
      TString pname;
      if ( bi_mht == 1 ) bi_mht = 2 ;
      pname.Form( "NJets%d_BTags%d_MHT%d_HT%d", bi_nj-1, bi_nb-1, bi_mht-2, bi_ht-1 ) ;


      ifstream ifs;
      ifs.open( "./new_bins.txt" ) ;
      ...
      while ( ifs.good() ) {
         ...
         char line_parname[100] ;
         int bi_no ;
         sscanf( line.Data(), "%d %s", &bi_no, line_parname ) ;
         if ( strcmp( line_parname, pname ) == 0 ) 
            return bi_no ;
      }

      std::cout << "\n\n *** get_par : Failed to find the bin number for parameter" << pname << std::endl ;
      gSystem -> Exit(-1) ;

      return 0 ; // To avoid the warning I see      

   } // find_new_region_bi_no
#endif
