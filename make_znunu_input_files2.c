...
#include "binning.h"
   ...
   void make_znunu_input_files2( const char* ldp_input_root_file = "non-qcd-inputs-fall16a/ZinvHistos_ldp.root",
         ...
         for ( int bi_nb=1; bi_nb<=nb_nb; bi_nb++ ) {
            for ( int bi_htmht=1; bi_htmht<=nb_htmht; bi_htmht++ ) {
               ...
               for ( int si=0; si<n_systerr; si++ ) {
                  ...
                  if ( bi_hist > 0 ) {
                     syst_lowdphi = h_systerr_lowdphi[si] -> GetBinContent( bi_hist ) ;
                     syst_highdphi = h_systerr_highdphi[si] -> GetBinContent( bi_hist ) ;
                  }
