...
void draw_badjet_cat_v3(const char* infile = "outputfiles/syst-2015-v2.root" ) {
   ...
   //    text -> DrawTextNDC( tx, ty, textcs ) ;
    ...
    //   DR and syst
      ...
      double rc_err = h_rprime -> GetBinError(1) ;
         ...
         fprintf( out_file     , "Sqcd_mhtc_%3s   1.000000     0.00000  0.00000\n", htstr) ;
         fprintf( out_file_data, "Sqcd_mhtc_%3s   1.000000     0.00000  0.00000\n", htstr) ;
      ...
      for ( int bi=2; bi<=nbins; bi++ ) {
        ...
        /// if the variable is equal to zero, equate it to the previous value in the list (if it is not the first variable of its kind)
         ...
         if ( dr_val == 0. && bi!=2)
         { 
            fprintf( out_file     , "Sqcd_mht%d_%3s   %3.6f     %3.5f  0.00000\n", bi-1, htstr, store_val, store_val) ;
            fprintf( out_file_data, "Sqcd_mht%d_%3s   %3.6f     %3.5f  0.00000\n", bi-1, htstr, store_val, store_val) ;
   ...
   {
         fprintf( out_file     , "Sqcd_nb%d        1.000000     0.00000  0.00000\n", nb_count) ;
         fprintf( out_file_data, "Sqcd_nb%d        1.000000     0.00000  0.00000\n", nb_count) ;
   }
...
bool transfer_qcd_parameters(string filein_name, string fileout_name)
{
    ...
    while( std::getline(filein,line) )
    {
        ...
        index = line.find("(");
        if (index == std::string::npos) { cout << "Warning: The structure of file " << filein_name << "is not as expected";filein.close();fileout.close(); return 0; }
        line.replace(line.find('('),line.find(')')-line.find('(')+1, "0.00000");
