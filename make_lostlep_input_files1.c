   ...
   void make_lostlep_input_files1(
                                 ...
                                 const char* hdp_input_root_file = "non-qcd-inputs-topup2/LLPrediction_QCD_HDP_Jul26_newSF.root",
         ...
         for ( int bi_nb=1; bi_nb<=nb_nb; bi_nb++ ) {
            for ( int bi_htmht=1; bi_htmht<=nb_htmht; bi_htmht++ ) {
               ...
               for ( int si=0; si<n_staterr; si++ ) {
                  double stat_lowdphi = h_staterr_lowdphi[si] -> GetBinContent( bi_hist ) - 1. ;
                  double stat_highdphi = h_staterr_highdphi[si] -> GetBinContent( bi_hist ) - 1. ;
               ...
               double combined_stat_err_lowdphi = sqrt( pow( total_stat_err2_lowdphi, 2. ) + pow( ldp_hist_err, 2. ) ) ;
               double total_syst_err_lowdphi = sqrt( total_syst_err2_lowdphi ) ;

               double combined_stat_err_highdphi = sqrt( pow( total_stat_err2_highdphi, 2. ) + pow( hdp_hist_err, 2. ) ) ;
      ...
      for ( int bi_ht=1; bi_ht<=nb_ht; bi_ht++ ) {
         for ( int bi_nj=1; bi_nj<=nb_nj; bi_nj++ ) {
            ...
            for ( int bi_nb=1; bi_nb<=nb_nb; bi_nb++ ) {
               ...
               double lowdphi_val = h_ldp -> GetBinContent( bi_hist ) ;
               double lowdphi_err = h_ldp -> GetBinError( bi_hist ) ;
