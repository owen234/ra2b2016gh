...
#include "histio.c"
   ...
   void draw_ratios_mc_and_model_v1( int draw_htbin = 3,
                                     ...
                                     const char* modelfile = "outputfiles/calc-model-ratios-v4.root",
                                     const char* plotdir = "outputfiles/model-mc-comp-v4/" ) {
    ...
    //---
      TCanvas* can = (TCanvas*) gDirectory -> FindObject( "can_draw_ratios_mc_and_model_v1" ) ;
      if ( can == 0x0 ) { can = new TCanvas( "can_draw_ratios_mc_and_model_v1", "", 900, 700 ) ; }
      can -> Clear() ;
    ...
    //---
      TCanvas* candr = (TCanvas*) gDirectory -> FindObject( "candr_draw_drs_mc_and_model_v1" ) ;
      if ( candr == 0x0 ) { candr = new TCanvas( "candr_draw_drs_mc_and_model_v1", "HT low", 900, 700 ) ; }
      candr -> Clear() ;
