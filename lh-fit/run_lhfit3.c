...
#include <sstream>
  ...
  using namespace RooStats;
   ...
   bool get_Rqcd_val_and_error( int hbi, int nji, float& Rqcd_val, float& Rqcd_err, RooFitResult* rfr, RooArgList& ral_rqcd_pars ) ;
  ...
  //---------------
   ...
   void run_lhfit1( const char* wsfile = "outputfiles/ws-lhfit-test3.root",
      ...
      {
         printf("\n\n\n ======= All workspace vars:\n") ;
         const RooArgList ws_all_vars = ws_pointer -> allVars() ;
         RooLinkedListIter vars_iter = ws_all_vars.iterator() ;
         while ( RooAbsReal* var = (RooAbsReal*) vars_iter.Next() ) {
            printf(" %50s : %12.4f ", var->GetName(), var->getVal()  ) ;
            double err = ((RooRealVar*) var) -> getError() ;
            if ( err > 0 ) {
               printf("+/- %12.4f\n", err ) ;
            } else {
               printf("\n") ;
      ...
      int num_model_pars(0.) ;
      {
         ...
         while ( RooAbsReal* var = (RooAbsReal*) vars_iter.Next() ) {
            ...
            if ( vname.Contains( "R_qcd_ldp_N" ) ) {
               num_model_pars ++ ;
               double err = ((RooRealVar*) var) -> getError() ;
               printf(" %50s : %12.4f +/- %12.4f\n", var->GetName(), var->getVal(), err  ) ;
            }
         } // var
         printf("\n\n") ;
      }
   ...
   // h_cor_mat -> Draw("text same") ;
      ...
      sprintf( output_file, "%s/correlation-matrix.pdf", output_dir ) ;
      can_cor_mat -> SaveAs( output_file ) ;
      ...
      for ( int pi=0; pi<float_pars_final.getSize(); pi++ ) {
         ...
         if ( pname.Index( "R_qcd" ) == 0 ) {
            rqcd_pars.add( *(float_pars_final.at(pi)) ) ;
            float val = ((RooAbsReal*)float_pars_final.at(pi))->getVal() ;
            float err = ((RooRealVar*)float_pars_final.at(pi))->getError() ;
            float rel_err(0.) ;
      ...
      TH1F* h_nobs_hdp = new TH1F( "h_nobs_hdp", "Nobs, hdp", n_bins_unblind, 0.5, n_bins_unblind + 0.5 ) ; hist_list.push_back( h_nobs_hdp ) ;

      TH1F* h_model_ldp = new TH1F( "h_model_ldp", "model, LDP", n_bins_unblind, 0.5, n_bins_unblind + 0.5 ) ; hist_list.push_back( h_model_ldp ) ;
      TH1F* h_model_hdp = new TH1F( "h_model_hdp", "model, hdp", n_bins_unblind, 0.5, n_bins_unblind + 0.5 ) ; hist_list.push_back( h_model_hdp ) ;
      ...
      TH1F* h_nonqcd_hdp = new TH1F( "h_nonqcd_hdp", "nonQCD, hdp", n_bins_unblind, 0.5, n_bins_unblind + 0.5 ) ; hist_list.push_back( h_nonqcd_hdp ) ;
      ...
      TH1F* h_nonqcdsub_hdp_statonly = new TH1F( "h_nonqcdsub_hdp_statonly", "Nobs-nonQCD, hdp", n_bins_unblind, 0.5, n_bins_unblind + 0.5 ) ; hist_list.push_back( h_nonqcdsub_hdp_statonly ) ;
      TH1F* h_nonqcdsub_hdp = new TH1F( "h_nonqcdsub_hdp", "Nobs-nonQCD, hdp", n_bins_unblind, 0.5, n_bins_unblind + 0.5 ) ; hist_list.push_back( h_nonqcdsub_hdp ) ;

      TH1F* h_lostlep_model_hdp = new TH1F( "h_lostlep_model_hdp", "lostlep hdp, model fit result", n_bins_unblind, 0.5, n_bins_unblind + 0.5 ) ; hist_list.push_back( h_lostlep_model_hdp ) ;
      TH1F* h_hadtau_model_hdp = new TH1F( "h_hadtau_model_hdp", "hadtau hdp, model fit result", n_bins_unblind, 0.5, n_bins_unblind + 0.5 ) ; hist_list.push_back( h_hadtau_model_hdp ) ;
      TH1F* h_znunu_model_hdp = new TH1F( "h_znunu_model_hdp", "znunu hdp, model fit result", n_bins_unblind, 0.5, n_bins_unblind + 0.5 ) ; hist_list.push_back( h_znunu_model_hdp ) ;
      TH1F* h_qcd_model_hdp = new TH1F( "h_qcd_model_hdp", "QCD hdp, model fit result", n_bins_unblind, 0.5, n_bins_unblind + 0.5 ) ; hist_list.push_back( h_qcd_model_hdp ) ;
      ...
      TH1F* h_sig0_hdp = new TH1F( "h_sig0_hdp", "SUSY at nominal Xsec, hdp", n_bins_unblind, 0.5, n_bins_unblind + 0.5 ) ; hist_list.push_back( h_sig0_hdp ) ;
      ...
      TH1F* h_pull_hdp = new TH1F( "h_pull_hdp", "(data-fit)/sqrt(data), hdp", n_bins_unblind, 0.5, n_bins_unblind + 0.5 ) ; hist_list.push_back( h_pull_hdp ) ;
...

      {
         ...
         for ( int hbi=1; hbi<=nb_ht[1]; hbi++ ) {
            for ( int nji=1; nji<=nb_nj; nji++ ) {
               ...
               char bin_name_underscore[1000] ;
               sprintf( bin_name, "Nj%d_HT%d", nji, hbi ) ;

               float Rqcd_val, Rqcd_err ;
               bool ok = get_Rqcd_val_and_error( hbi, nji, Rqcd_val, Rqcd_err, fitResult,  rqcd_pars ) ;
               if ( !ok ) continue ;
               ...
               RooAbsReal* rar_Nhdp = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rar_Nhdp == 0x0 ) { printf("\n\n *** ws missing %s\n\n", pname ) ; return ; }
               float Nhdp_val = rar_Nhdp -> getVal() ;

               printf(" %2d %30s  :  Nldp = %8.1f   Nhdp = %8.1f\n", bi, bin_name, nldp_val, Nhdp_val ) ;
               ...
               RooAbsReal* rar_model_Nhdp = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rar_model_Nhdp == 0x0 ) { printf("\n\n *** ws missing %s\n\n", pname ) ; return ; }
               float model_Nhdp_val = rar_model_Nhdp -> getVal() ;
               ...
               double lostlep_hdp_val0(-1.) ;
               sprintf( pname, "mean_mu_lostlep_hdp_statonly_%s", bin_name ) ;
               RooAbsReal* rv_lostlep_hdp_val0 = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_lostlep_hdp_val0 != 0x0 ) lostlep_hdp_val0 = rv_lostlep_hdp_val0 -> getVal() ;

               double lostlep_hdp_val(-1.) ;
               sprintf( pname, "mu_lostlep_hdp_%s", bin_name ) ;
               RooAbsReal* rv_lostlep_hdp_val = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_lostlep_hdp_val != 0x0 ) lostlep_hdp_val = rv_lostlep_hdp_val -> getVal() ;

               double lostlep_hdp_err_stat(-1.) ;
               sprintf( pname, "sigma_mu_lostlep_hdp_statonly_%s", bin_name ) ;
               RooAbsReal* rv_lostlep_hdp_err_stat = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_lostlep_hdp_err_stat != 0x0 ) lostlep_hdp_err_stat = rv_lostlep_hdp_err_stat -> getVal() ;

               ///----- fixme
               double lostlep_hdp_err_syst(-1.) ;
               sprintf( pname, "sigma_mu_lostlep_hdp_systerr_%s", bin_name ) ;
               RooAbsReal* rv_lostlep_hdp_err_syst = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_lostlep_hdp_err_syst != 0x0 ) lostlep_hdp_err_syst = rv_lostlep_hdp_err_syst -> getVal() ;
               lostlep_hdp_err_syst = lostlep_hdp_err_syst * lostlep_hdp_val0 ;
               ///----- fixme

               double lostlep_hdp_err = sqrt( lostlep_hdp_err_stat*lostlep_hdp_err_stat + lostlep_hdp_err_syst*lostlep_hdp_err_syst ) ;


               printf("  %2d  HDP %30s : lost lep, mean0 = %6.1f +/- %5.1f,  val = %6.1f\n",
                  bi, bin_name, lostlep_hdp_val0, lostlep_hdp_err, lostlep_hdp_val ) ;





               double hadtau_hdp_val0(-1.) ;
               sprintf( pname, "mean_mu_hadtau_hdp_statonly_%s", bin_name ) ;
               RooAbsReal* rv_hadtau_hdp_val0 = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_hadtau_hdp_val0 != 0x0 ) hadtau_hdp_val0 = rv_hadtau_hdp_val0 -> getVal() ;

               double hadtau_hdp_val(-1.) ;
               sprintf( pname, "mu_hadtau_hdp_%s", bin_name ) ;
               RooAbsReal* rv_hadtau_hdp_val = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_hadtau_hdp_val != 0x0 ) hadtau_hdp_val = rv_hadtau_hdp_val -> getVal() ;

               double hadtau_hdp_err_stat(-1.) ;
               sprintf( pname, "sigma_mu_hadtau_hdp_statonly_%s", bin_name ) ;
               RooAbsReal* rv_hadtau_hdp_err_stat = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_hadtau_hdp_err_stat != 0x0 ) hadtau_hdp_err_stat = rv_hadtau_hdp_err_stat -> getVal() ;

               ///----- fixme
               double hadtau_hdp_err_syst(-1.) ;
               sprintf( pname, "sigma_mu_hadtau_hdp_systerr_%s", bin_name ) ;
               RooAbsReal* rv_hadtau_hdp_err_syst = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_hadtau_hdp_err_syst != 0x0 ) hadtau_hdp_err_syst = rv_hadtau_hdp_err_syst -> getVal() ;
               hadtau_hdp_err_syst = hadtau_hdp_err_syst * hadtau_hdp_val0 ;
               ///----- fixme

               double hadtau_hdp_err = sqrt( hadtau_hdp_err_stat*hadtau_hdp_err_stat + hadtau_hdp_err_syst*hadtau_hdp_err_syst ) ;

               printf("  %2d  HDP %30s : had tau,  mean0 = %6.1f +/- %5.1f,  val = %6.1f\n",
                  bi, bin_name, hadtau_hdp_val0, hadtau_hdp_err, hadtau_hdp_val ) ;





               double znunu_hdp_val0(-1.) ;
               sprintf( pname, "mean_mu_znunu_hdp_statonly_%s", bin_name ) ;
               RooAbsReal* rv_znunu_hdp_val0 = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_znunu_hdp_val0 != 0x0 ) znunu_hdp_val0 = rv_znunu_hdp_val0 -> getVal() ;

               double znunu_hdp_val(-1.) ;
               sprintf( pname, "mu_znunu_hdp_%s", bin_name ) ;
               RooAbsReal* rv_znunu_hdp_val = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_znunu_hdp_val != 0x0 ) znunu_hdp_val = rv_znunu_hdp_val -> getVal() ;

               double znunu_hdp_err_stat(-1.) ;
               sprintf( pname, "sigma_mu_znunu_hdp_statonly_%s", bin_name ) ;
               RooAbsReal* rv_znunu_hdp_err_stat = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_znunu_hdp_err_stat != 0x0 ) znunu_hdp_err_stat = rv_znunu_hdp_err_stat -> getVal() ;

               ///----- fixme
               double znunu_hdp_err_syst(-1.) ;
               sprintf( pname, "sigma_mu_znunu_hdp_systerr_%s", bin_name ) ;
               RooAbsReal* rv_znunu_hdp_err_syst = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_znunu_hdp_err_syst != 0x0 ) znunu_hdp_err_syst = rv_znunu_hdp_err_syst -> getVal() ;
               znunu_hdp_err_syst = znunu_hdp_err_syst * znunu_hdp_val0 ;
               ///----- fixme

               double znunu_hdp_err = sqrt( znunu_hdp_err_stat*znunu_hdp_err_stat + znunu_hdp_err_syst*znunu_hdp_err_syst ) ;

               printf("  %2d  HDP %30s : znunu,    mean0 = %6.1f +/- %5.1f,  val = %6.1f\n",
                  bi, bin_name, znunu_hdp_val0, znunu_hdp_err, znunu_hdp_val ) ;




               double sig0_hdp_val(-1.) ;
               sprintf( pname, "mu_sig0_hdp_%s", bin_name ) ;
               RooAbsReal* rv_sig0_hdp = (RooAbsReal*) ws_pointer -> obj( pname ) ;
               if ( rv_sig0_hdp != 0x0 ) sig0_hdp_val = rv_sig0_hdp -> getVal() ;
               ...
               float nonqcd_hdp_val = lostlep_hdp_val + hadtau_hdp_val + znunu_hdp_val ;
               float nonqcd_hdp_err = sqrt( lostlep_hdp_err*lostlep_hdp_err + hadtau_hdp_err*hadtau_hdp_err + znunu_hdp_err*znunu_hdp_err ) ;

               float nonqcdsub_ldp_val = nldp_val - nonqcd_ldp_val ;
               float nonqcdsub_ldp_err = sqrt( nldp_val + nonqcd_ldp_err*nonqcd_ldp_err ) ;

               float nonqcdsub_hdp_val = Nhdp_val - nonqcd_hdp_val ;
               float nonqcdsub_hdp_err = sqrt( Nhdp_val + nonqcd_hdp_err*nonqcd_hdp_err ) ;
               ...
               double qcd_hdp_val = qcd_ldp_val * Rqcd_val ;
               double qcd_hdp_err(0.) ;
               if ( qcd_ldp_val > 0. && Rqcd_val > 0. ) {
                  qcd_hdp_err = qcd_hdp_val * sqrt( pow( qcd_ldp_err/qcd_ldp_val, 2 ) + pow( Rqcd_err/Rqcd_val, 2 ) ) ;
               }
               printf(" %2d  %30s : QCD LDP %7.1f +/- %5.1f,   QCD hdp %7.1f +/- %5.1f\n",
                  bi, bin_name, qcd_ldp_val, qcd_ldp_err, qcd_hdp_val, qcd_hdp_err ) ;


               double pull_hdp_val(0.) ;
               if ( Nhdp_val > 0 ) {
                  pull_hdp_val = (Nhdp_val - model_Nhdp_val) / sqrt(Nhdp_val) ;
               }
               ...
               h_nobs_hdp  -> SetBinContent( bi, Nhdp_val ) ;

               h_model_ldp -> SetBinContent( bi, model_nldp_val ) ;
               h_model_hdp  -> SetBinContent( bi, model_Nhdp_val ) ;

               h_nonqcd_ldp -> SetBinContent( bi, nonqcd_ldp_val ) ;
               h_nonqcd_ldp -> SetBinError( bi, nonqcd_ldp_err ) ;

               h_nonqcd_hdp -> SetBinContent( bi, nonqcd_hdp_val ) ;
               h_nonqcd_hdp -> SetBinError( bi, nonqcd_hdp_err ) ;

               h_nonqcdsub_hdp -> SetBinContent( bi, nonqcdsub_hdp_val ) ;
               h_nonqcdsub_hdp -> SetBinError( bi, nonqcdsub_hdp_err ) ;

               h_nonqcdsub_hdp_statonly -> SetBinContent( bi, nonqcdsub_hdp_val ) ;
               h_nonqcdsub_hdp_statonly -> SetBinError( bi, sqrt(Nhdp_val) ) ;
               ...
               h_lostlep_model_hdp -> SetBinContent( bi, lostlep_hdp_val ) ;
               h_lostlep_model_hdp -> SetBinError( bi, lostlep_hdp_err ) ;

               h_hadtau_model_hdp -> SetBinContent( bi, hadtau_hdp_val ) ;
               h_hadtau_model_hdp -> SetBinError( bi, hadtau_hdp_err ) ;

               h_znunu_model_hdp -> SetBinContent( bi, znunu_hdp_val ) ;
               h_znunu_model_hdp -> SetBinError( bi, znunu_hdp_err ) ;

               h_qcd_model_hdp -> SetBinContent( bi, qcd_hdp_val ) ;
               h_qcd_model_hdp -> SetBinError( bi, qcd_hdp_err ) ;

               h_qcd_model_ldp -> SetBinContent( bi, qcd_ldp_val ) ;
               h_qcd_model_ldp -> SetBinError( bi, qcd_ldp_err ) ;

               h_pull_hdp -> SetBinContent( bi, pull_hdp_val ) ;
               h_pull_ldp -> SetBinContent( bi, pull_ldp_val ) ;

               h_sig0_hdp -> SetBinContent( bi, sig0_hdp_val ) ;
     ...
     //--- Read in QCD MC counts, if the file is provided, and include the HDP counts in the tables.
      ...
      if ( include_qcdmc ) {
         ...
         fprintf( ofp_qcd_table1, "  &  \\multicolumn{3}{|c||}{  LDP }   &  \\multicolumn{4}{|c|}{ hdp }  \\\\\n" ) ;
         ...
         fprintf( ofp_qcd_table2, "  &  \\multicolumn{3}{|c||}{  LDP }   &  \\multicolumn{4}{|c|}{ hdp }  \\\\\n" ) ;
         ...
         fprintf( ofp_qcd_table3, "  &  \\multicolumn{2}{|c||}{  LDP } &    &  \\multicolumn{3}{|c|}{ hdp }  \\\\\n" ) ;
         ...
         fprintf( ofp_qcd_table1, "\\hline\n" ) ;
         fprintf( ofp_qcd_table1, "  &  \\multicolumn{3}{|c||}{  LDP }   &  \\multicolumn{3}{|c|}{ hdp }  \\\\\n" ) ;
         ...
         fprintf( ofp_qcd_table2, "\\hline\n" ) ;
         fprintf( ofp_qcd_table2, "  &  \\multicolumn{3}{|c||}{  LDP }   &  \\multicolumn{3}{|c|}{ hdp }  \\\\\n" ) ;
         ...
         fprintf( ofp_qcd_table3, "\\hline\n" ) ;
         fprintf( ofp_qcd_table3, "  &  \\multicolumn{2}{|c||}{  LDP } &    &  \\multicolumn{2}{|c|}{ hdp }  \\\\\n" ) ;
      ...
      for ( int bi=1; bi<=n_bins_unblind; bi++ ) {
         ...
         TString bin_name_dash( short_bin_name ) ;
         bin_name_dash.ReplaceAll("_","-") ;
         ...
         fprintf( ofp_qcd_table1,  " %20s &  $%7.0f$  &  $%7.1f$  &  $%7.1f \\pm %5.1f$ &  $%7.0f$  &  $%7.1f$  &  $%7.1f \\pm %5.1f$  ",
              ...
              h_nobs_hdp -> GetBinContent(bi),
              h_model_hdp -> GetBinContent(bi),
              h_qcd_model_hdp -> GetBinContent(bi),  h_qcd_model_hdp -> GetBinError(bi) ) ;
         ...
         fprintf( ofp_qcd_table2,  " %20s &  $%7.0f$  &  $%7.1f \\pm %5.1f$  &  $%7.1f \\pm %5.1f$ &  $%7.0f$  &  $%7.1f \\pm %5.1f$  &  $%7.1f \\pm %5.1f$  ",
              ...
              h_qcd_model_ldp -> GetBinContent(bi),  h_qcd_model_ldp -> GetBinError(bi),
              h_nobs_hdp -> GetBinContent(bi),
              h_nonqcd_hdp -> GetBinContent(bi),  h_nonqcd_hdp -> GetBinError(bi),
              h_qcd_model_hdp -> GetBinContent(bi),  h_qcd_model_hdp -> GetBinError(bi) ) ;
         ...
         fprintf( ofp_qcd_table3,  " %20s &  $%7.1f \\pm %5.1f$  &  $%7.1f \\pm %5.1f$ &    $%5.3f \\pm %5.3f$  &  $%7.1f \\pm %5.1f$  &  $%7.1f \\pm %5.1f$  ",
              ...
              h_nonqcdsub_hdp -> GetBinContent(bi),  h_nonqcdsub_hdp -> GetBinError(bi),
              h_qcd_model_hdp -> GetBinContent(bi),  h_qcd_model_hdp -> GetBinError(bi) ) ;

         if ( include_qcdmc) {
            float qcdmc_hdp_val(0.), qcdmc_hdp_err(0.) ;
            ////////////get_qcdmc_counts( short_bin_name, qcdmc_hdp_val, qcdmc_hdp_err ) ;
            get_qcdmc_counts( bin_name_dash.Data(), qcdmc_hdp_val, qcdmc_hdp_err ) ;
      ...
      if ( can_qcd == 0x0 ) {
         can_qcd = new TCanvas( "can_qcd", "QCD, hdp fit result", 1000, 700 ) ;
      }
      can_qcd -> cd() ;

      h_qcd_model_hdp -> Draw() ;
  ...
  //==========================================================================================
   ...
   bool get_Rqcd_val_and_error( int hbi, int nji, float& Rqcd_val, float& Rqcd_err, RooFitResult* rfr, RooArgList& ral_rqcd_pars ) {
      ...
      char pname[100] ;
      sprintf( pname, "R_qcd_ldp_Nj%d_HT%d", nji, hbi ) ;
      RooRealVar* rrv_Rqcd = (RooRealVar*) ral_rqcd_pars.find( pname ) ;
      if ( rrv_Rqcd == 0x0 ) {
         printf("\n\n *** get_Rqcd_val_and_error: can't find fit par %s\n\n\n", pname ) ;
         gSystem -> Exit( -1 ) ;
      } else {
         Rqcd_val =  rrv_Rqcd -> getVal() ;
         Rqcd_err =  rrv_Rqcd -> getError() ;
      }
  ...
  //---------
   ...
   void draw_stack( const char* bin_name_substring, int yaxis_option ) {
      ...
      sprintf( hname, "h_nobs_hdp%s", histend ) ;
      TH1F* h_nobs = get_hist( hname ) ;

      sprintf( hname, "h_lostlep_model_hdp%s", histend ) ;
      TH1F* h_lostlep = get_hist( hname ) ;
      sprintf( hname, "h_hadtau_model_hdp%s", histend ) ;
      TH1F* h_hadtau = get_hist( hname ) ;
      sprintf( hname, "h_znunu_model_hdp%s", histend ) ;
      TH1F* h_znunu = get_hist( hname ) ;
      sprintf( hname, "h_qcd_model_hdp%s", histend ) ;
      TH1F* h_qcd = get_hist( hname ) ;

      sprintf( hname, "h_model_hdp%s", histend ) ;
      TH1F* h_all = get_hist( hname ) ;

      sprintf( hname, "h_sig0_hdp%s", histend ) ;
  ...
  //---------
   ...
   void draw_nonqcdsub( const char* bin_name_substring ) {
      ...
      sprintf( hname, "h_nonqcdsub_hdp_statonly%s", histend ) ;
      TH1F* h_nonqcdsub_statonly = get_hist( hname ) ;

      sprintf( hname, "h_nonqcdsub_hdp%s", histend ) ;
  ...
  //---------
   ...
   void draw_pull( const char* bin_name_substring ) {
      ...
      sprintf( hname, "h_pull_hdp%s", histend ) ;
