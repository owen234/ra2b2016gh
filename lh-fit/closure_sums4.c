#ifndef closure_sums4_c
#define closure_sums4_c
...
#include "../num_to_str.h"
  ...
  //----
   ...
   void closure_sums4(
          ...
          const char* model_pars_file2 = "../outputfiles/kqcd-parameters-from-qcdmc.txt",
          const char* model_ratio_hist_file = "outputfiles/model-ratio-hist4.root",
     ...
     //--- boxes
      ...
      if ( include_qcdmc_correction ) {
         saveHist( "outputfiles/closure-sums4-wcor.root", "h*" ) ;
      } else {
         saveHist( "outputfiles/closure-sums4.root", "h*" ) ;
      }
   ...
   void calc_sum( bool verb ) {
         ...
         for ( int sbi=0; sbi<sum_nbins; sbi++ ) {
           ...
           //--- skip HT1 at high Njet.
            ...
            model_ratio =  par_val_ht_njet[bi_ht][bi_nj] * par_val_ht_mht[bi_ht][bi_mht] * par_val_nb[bi_nb] ;
         ...
         float partial_sum_ht_njet[10][10]{} ;
        ...
        ///    partial_sum[2][bi_nj] += model_val / par_val_njet[bi_nj] ;
            ...
            if ( par_val_ht_njet[bi_ht][bi_nj] > 0. )
               partial_sum_ht_njet[bi_ht][bi_nj] += model_val / par_val_ht_njet[bi_ht][bi_nj] ;
         ...
         float par_err_fit_ht_njet[10][10] ;
         float par_err_syst_ht_njet[10][10] ;
         char  par_name_ht_njet[10][10][36] ;
      ...
      ////  }//bin_nj
            ...
            for ( int bi_ht = 1; bi_ht <= nBinsHT; bi_ht++) {
               for ( int bi_nj = 1; bi_nj <= nb_nj; bi_nj++) {
                  //--- skip HT1 at high Njet.
                   if ( bi_ht == 1 && bi_nj > nb_nj-2 ) continue ;
                   par_err_fit_ht_njet[bi_ht][bi_nj] = par_err_ht_njet[bi_ht][bi_nj] ;
                   par_err_syst_ht_njet[bi_ht][bi_nj] = 0. ;
                   sprintf( par_name_ht_njet[bi_ht][bi_nj], "Nj%d_HT%d", bi_nj, bi_ht ) ;
                   printf( " *** Debug1 : bi_ht=%d, bi_nj=%d : par_name = %s , par_err_fit_ht_njet = %6.4f\n", bi_ht, bi_nj, par_name_ht_njet[bi_ht][bi_nj], par_err_fit_ht_njet[bi_ht][bi_nj] ) ;
               }//bi_nj
            }//bi_ht
         ...
         for ( int bi_ht = 1; bi_ht <= nBinsHT; bi_ht++) {
            for ( int bi_nj = 1; bi_nj <= nb_nj; bi_nj++) {
               //--- skip HT1 at high Njet.
                if ( bi_ht == 1 && bi_nj > nb_nj-2 ) continue ;
                model_sum_err2_fit += pow( partial_sum_ht_njet[bi_ht][bi_nj] * par_err_fit_ht_njet[bi_ht][bi_nj], 2. ) ;
                if (verb) {
                      printf("  %5s : partial sum %8.1f, err %8.4f , contrib %8.1f\n",
                            par_name_ht_njet[bi_ht][bi_nj], partial_sum_ht_njet[bi_ht][bi_nj], par_err_fit_ht_njet[bi_ht][bi_nj],
                            partial_sum_ht_njet[bi_ht][bi_nj] * par_err_fit_ht_njet[bi_ht][bi_nj] ) ;
                } //verb
            }//bi_nj
         }//bi_ht
  ...
  //=======================================================================================
   ...
   void print_search_bin_table() {
	 ...
	 hbi++;
         ...
         if ( fabs(model_ratio-model_ratio_from_hist) > 0.00001 ) {
            printf("\n *** inconsistent model ratios in Njet bin = %d, Nbjet = bin %d, HT bin = %d and MHT bin = %d.\n\n", bi_nj, bi_nb, bi_ht, bi_mht ) ;
            printf("         calculated here: %6.4f ,   from hist: %6.4f\n", model_ratio, model_ratio_from_hist ) ;
         }
