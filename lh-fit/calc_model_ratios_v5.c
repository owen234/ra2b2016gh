  ...
  //--------
   ...
   void calc_model_ratios_v5(
      ...
      for ( int bi_ht = 1; bi_ht <= nBinsHT; bi_ht++) {
         ...
         h_model_ratio[bi_ht][nb_nb+1] = new TH1F( "h_model_ratio_"+ht_str+"_nbsum", "Model, H/L ratio, Nbsum"       +ht_str2, nbins, 0.5, nbins+0.5 ) ;
         h_model_dr   [bi_ht][nb_nb+1] = new TH1F( "h_model_dr_"   +ht_str+"_nbsum", "Model, double H/L ratio, Nbsum"+ht_str2, nbins, 0.5, nbins+0.5 ) ;
         ...
         for ( int nbi = 1; nbi <= (nb_nb+1) ; nbi++ ) {
            for ( int mbi = 1; mbi <= nb_mht; mbi++ ) {
               ...
               for ( int hti = htb_low; hti<=nBinsHT; hti++ ) {
                  ...
                  model_ratio_val =  par_val_ht_njet[hti][nji]  * par_val_mht * par_val_nb[nbi] ;
                  ...
                  if ( par_val_ht_njet[hti][nji]   > 0 )   model_ratio_err2 += pow( model_ratio_val * par_err_ht_njet[hti][nji]    / par_val_ht_njet[hti][nji]  , 2. ) ;
                  ...
                  if ( mbi > 1 && nbi <= nb_nb ) {
                   ...
                   printf("  %3d :  Nj%d Nb%d MHT%d HT%d :  HT-Nj %7.4f * MHT %7.4f * Nb %7.4f  = %7.4f +/- %7.4f\n",
                          print_bi, nji, nbi-1, mbi, hti,
                          par_val_ht_njet[hti][nji], par_val_mht, par_val_nb[nbi],
                          model_ratio_val, model_ratio_err ) ;
      ...
      saveHist( "outputfiles/calc-model-ratios-v5.root", "h*" ) ;


   } // calc_model_ratios_v5
