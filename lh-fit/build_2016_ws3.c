...
///   }
      ...
      while ( ifs_data.good() ) {
       ...
       //-- data
         ...
         char bin_name_underscore[100] ;
         sprintf( bin_name_underscore, "Nj%d_HT%d", fb_nji, fb_hbi ) ; //-- no minus sign in the name.
...
//       }
  ...
  ///    rv_R_qcd_ldp -> Print() ;
         ...
         sprintf( pname, "Nldp_%s", bin_name_underscore ) ;
         ...
         sprintf( pname, "Nhdp_%s", bin_name_underscore ) ;
       ...
       //-- sigmc
         ...
         sprintf( pname, "mu_sig0_ldp_%s", bin_name_underscore ) ;
         ...
         sprintf( pname, "mu_sig_hdp_%s", bin_name_underscore ) ;
         ...
         sprintf( pname, "mu_qcd_ldp_%s", bin_name_underscore ) ;
         ...
         sprintf( pname, "mu_qcd_hdp_%s", bin_name_underscore ) ;
         ...
         sprintf( pname, "pdf_ldp_%s", bin_name_underscore ) ;
         ...
         sprintf( pname, "pdf_hdp_%s", bin_name_underscore ) ;
  ...
  //=================================================================================
   ...
   RooAbsReal* read_bg_line( ifstream& ifs, const char* bin_name ) {
      ...
      TString tsbn( bin_name ) ;
      tsbn.ReplaceAll("-","_") ;


      char pname[1000] ;
      sprintf( pname, "mu_%s_%s_statonly_%s", bg_name, region_name, tsbn.Data() ) ;
      RooAbsReal* rv_mu_statonly = makeLognormalConstraint( pname, val, stat_err ) ;
      //RooAbsReal* rv_mu_statonly = makeGaussianConstraint( pname, val, stat_err ) ;
      ...
      for ( int si=0; si<nsyst; si++ ) {
         ...
         sprintf( pname, "mu_%s_%s_systerr_%s_%s", bg_name, region_name, syst_name[si], tsbn.Data() ) ;
         ...
         ////RooAbsReal* rv_mu_systerr = makeCorrelatedGaussianConstraint( pname, 1.0, syst_err_rel, systparname ) ;
         ras_product_terms -> add( *rv_mu_systerr ) ;
         char tmpstr[1000] ;
         sprintf( tmpstr, "%s", formula_string ) ;
         sprintf( formula_string, "%s * @%d", tmpstr, si+1 ) ;
      } // si
      sprintf( pname, "mu_%s_%s_%s", bg_name, region_name, tsbn.Data() ) ;
