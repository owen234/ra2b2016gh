...
#ifndef fill_hists_loop_v2d_cxx
...
void fill_hists_loop_v2d::Loop( bool verb, int nloop )
{
   ...
   TH1F* h_mhtc_nbsum_hdp = new TH1F( "h_mhtc_nbsum_hdp", "HDP events, MHTC, Nbsum", no_bin_mhtc_nbsum, 0.5, no_bin_mhtc_nbsum+0.5 ) ;
   set_bin_labels_mhtc_nbsum_plot( h_mhtc_nbsum_hdp ) ;

   TH1F* h_mhtc_nbsum_ldp = new TH1F( "h_mhtc_nbsum_ldp", "ldp events, MHTC, Nbsum", no_bin_mhtc_nbsum, 0.5, no_bin_mhtc_nbsum+0.5 ) ;
   ...
   for (Long64_t jentry=0; jentry<loopmax;jentry++) 
   {
    ...
    //-- new baseline
     ...
     /////////////
      ...
      if ( bi_htmht > 0 ) {translate_htmht_bin_to_ht_and_mht_bins (bi_htmht, bi_ht, bi_mht);}
      else                continue;
      ...
      int bi_mhtc_nbsum_plot = global_bin_only_mhtc_sum_nb ( bi_nj, bi_htmht ) ;
   ...
   TH1F* h_max_ldp_weight = new TH1F( "h_max_ldp_weight", "Max ldp weight", nb_global_w_exclusion_w_mhtc, 0.5, nb_global_w_exclusion_w_mhtc + 0.5 ) ;
   sbi = 0;
   for ( int i=1; i<=nb_global; i++ ) {
      if ( is_this_bin_excluded(i) ) continue;
      sbi++;
...
//=====================================================================================================
   ...
   void fill_hists_loop_v2d::set_bin_labels_nb ( TH1F* hp , int tbi_nb) {
   ...
   void fill_hists_loop_v2d::set_bin_labels_mhtc_nbsum_plot( TH1F* hp ) {
      ...
      for ( int tbi_nj=1; tbi_nj<=nb_nj; tbi_nj++ ) {
            for ( int tbi_ht=1; tbi_ht<=(nb_ht[1]); tbi_ht++ ) {
               ...
               if ( is_this_bin_excluded_nbsum (tbi_nj-1, tbi_ht-1, 0 ) ) continue;
     	       bi_plot ++ ;
               char binlabel[100] ;
